...
export default function CreateHiveCard() {
  ...
  const userId = 1 // <- você pode puxar isso do contexto do usuário autenticado
  ...
  const handleCriar = async (dados) => {
    ...
    const payload = {
      user_id: userId,
      bee_type_id: parseInt(dados.beeType), // beeType é o ID vindo do select
      location_lat: dados.location?.lat || 0,
      location_lng: dados.location?.lng || 0,
      size: dados.size,
      humidity: 0,
      temperature: 0
    }
    ...
    try {
      const response = await axios.post('http://host.docker.internal:8000/hives/create',{
  method: "POST",
  headers: {
